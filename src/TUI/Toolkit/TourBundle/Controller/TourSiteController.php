...
class TourSiteController extends Controller
{
  ...
  public function siteShowAction($id, $quoteNumber = null)
  {
    ...
    // if organizer or if brand or higher (check permission table for organizer)
        ...
        $entity = $em->getRepository('TourBundle:QuoteVersion')->find($id);
        if (!$entity) {
    ...
    if($quoteNumber===NULL && FALSE === $securityContext->isGranted('ROLE_BRAND')){
      ...
      return $this->render('TourBundle:QuoteSite:sitePrompt.html.twig', array(
        'entity'      => $entity,
    ...
    $versions = $em->getRepository('TourBundle:QuoteVersion')->findBy(array('quoteReference' => $qr));
    if(count($versions) > 1){
      $alternate=TRUE;
    }


    return $this->render('TourBundle:QuoteSite:siteShow.html.twig', array(
      'entity'      => $entity,
      'locale'      => $locale,
      'items'       => $items,
      'tabs'        => $tabs,
  ...
  public function setQuoteViews($id) {
    ...
    public function newRequestAction($id)
    {
        ...
        return $this->render('TourBundle:QuoteSite:changeRequest.html.twig', array(
    ...
    public function requestChangeAction(Request $request, $id)
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $entity = $em->getRepository('TourBundle:QuoteVersion')->findById($id);
        $entity = $entity[0];
        ...
        $message = \Swift_Message::newInstance()
            ...
            ->setBody(
                $this->renderView(
                    'TourBundle:Emails:changerequest.html.twig',
                    array(
                        'brand' => $brand,
                        'entity' => $entity,
                        'changes' => $changes,
    ...
    public function newAcceptAction($id)
    {
        ...
        return $this->render('TourBundle:QuoteSite:acceptQuote.html.twig', array(
    ...
    public function quoteAcceptedAction(Request $request, $id)
    {
        ...
        $message = \Swift_Message::newInstance()
            ...
            ->setBody(
                $this->renderView(
                    'TourBundle:Emails:acceptQuote.html.twig',
                    array(
                        'brand' => $brand,
                        'entity' => $entity,
                        'departure' => $departure,
    ...
    public function sitePDFAction($id, $quoteNumber = null)
    {
      ...
      // prepare Alternate Quotes flag.
      $versions = $em->getRepository('TourBundle:QuoteVersion')->findByQuoteReference($entity[0]->getQuoteReference());
      if(count($versions>1)){
        $alternate=TRUE;
      }
      ...
      $html = $this->renderView( 'TourBundle:QuoteSite:sitePDF.html.twig', $data );
      ...
      /*
      return $this->render('TourBundle:QuoteSite:sitePDF.html.twig', array(
        'entity'      => $entity[0],
        'locale'      => $locale,
        'items'       => $items,
        'warning'     => $warningMsg,
  ...
  public function getQuoteSiblingsAction($id)
  {
    $locale = $this->container->getParameter('locale');
    $em = $this->getDoctrine()->getManager();
    $tour = $em->getRepository('TourBundle:Quote')->find($id);
    if(!$tour){
      throw $this->createNotFoundException('Unable to find Quote entity for alternate quotes tab.');
    }
    $tourName = $tour->getName();
    $entities = $em->getRepository('TourBundle:QuoteVersion')->findByQuoteReference($id);

    if(!$entities){
      throw $this->createNotFoundException('Unable to find QuoteVersion entities for alternate quotes tab.');
    }

    return $this->render('TourBundle:QuoteSite:quoteSiblings.html.twig', array(
      'entities' => $entities,
      'locale'  => $locale,
      'tourName'  => $tourName,
    ));
  ...
  public function showSummaryAction($id)
  {
    $locale = $this->container->getParameter('locale');
    $em = $this->getDoctrine()->getManager();
    $quote = $em->getRepository('TourBundle:QuoteVersion')->find($id);
    if(!$quote){
      throw $this->createNotFoundException('Unable to find Quote entity for summary header display.');
    }

    return $this->render('TourBundle:QuoteSite:quoteSummary.html.twig', array(
      'quote' => $quote,
      'locale'  => $locale,
    ));
  }
  ...
  public function editSummaryAction($id) {
    ...
    return $this->render('TourBundle:QuoteSite:editSummary.html.twig', array(
      'entity' => $entity,
  ...
  public function updateSummaryAction(Request $request, $id)
  {
    ...
    return $this->render('TourBundle:QuoteVersion:editSummary.html.twig', array(
      'entity' => $entity,
      'edit_form' => $editForm->createView(),
